...
- [ ] 🔴 **`dataStore.load()` distrugge personalizzazioni utente a ogni reload** — `src/engine/dataStore.ts:228-232`: riordina i link secondo DEFAULT_CONFIG → drag&drop e Move Up/Down annullati al refresh. `dataStore.ts:197-226`: forza `title/category/searchPath/aliases` dei link di default → rename utente perso. Gli hack di migrazione hardcoded per titolo ('Orari'/'Esami', righe 216-225) impediscono di editare quei link dal VisualEditModal e violano la Decoupled Data Architecture (gemini.md §1.3).
