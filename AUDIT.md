...
>
> **Stato attuale misurato (2026-08-03, post-merge fasi R1+R2+R3 su dev):** 30 file test / **241 test PASS** · `tsc --noEmit` = **0 errori** · `bun run lint` = 0 · `dist/index.html` = **263.18 KB** (~80.26 KB gzip) · budget build 350 KB attivo in `productionBuild.test.ts` · P0-P6 completati · **P7 (igiene repo) completato** (R1 docs/hygiene, R2 CSS/design-tokens, R3 engine hardening): **unico item aperto residuo: 🟠 P4 lucide-preact lazy catalog (R4 in corso).**
> **Stato R4 (2026-08-03, branch `perf/audit-lucide-bundle`):** sostituzione `lucide-preact` → registry locale `Icon.tsx`/`iconPaths.ts` completata; **30 file test / 241 test PASS**, `tsc` 0, `lint` 0, dist `index.html` **240.82 KB** (~72.20 KB gz) → **P4 risolto, budget 350 KB soddisfatto con ampio margine**. In attesa di merge su dev e rilascio R5 (v0.4.0).
...
- [ ] 🟠 **Bundle: lucide-preact ancora dominante (123 KB raw / 75.4 KB gz)** — dal visualizer `dist/stats.html` (misurato 2026-08-03 su dev post-P6): `src` 190 KB raw / 54.9 KB gz, **lucide 123 KB raw / 75.4 KB gz**, node_modules(altri) 57.2 KB raw / 15.7 KB gz, preact 21 KB raw / 8.8 KB gz. Il primo fix P0 (registry statico con named import) ha abbassato il bundle a ~268 KB, ma lucide resta il peso maggiore dopo `src` — segnale di codice duplicato nei componenti importati e/o import non tree-shaken. Dist totale 268.58 KB (79.85 KB gz) vs budget 350 KB: margine ~82 KB.
  **Fix:** ✅ **Completato (R4):** sostituzione integrale di `lucide-preact` con un registry locale di path-data SVG statici. Generatore `scripts/extractIcons.mjs` estrae l'inner markup SVG dalle icone realmente usate → `src/components/iconPaths.ts` (48.7 KB, 240 icone — niente più 1767). Renderer `src/components/Icon.tsx` replica la resa lucide (viewBox 24px, stroke-width 2, round caps, aria-hidden/title). `iconRegistry.ts` riscritto su `ICON_PATHS` (`getLucideIcon`/`isLucideIconSpec`/`searchLucideIcons`, 240 voci). Migrate 16 componenti TSX `import {X} from 'lucide-preact'` → `import {Icon} from './Icon'` (ActionToolbar usa `iconComponentFor()` per icone come valori). Verifica: `tsc` 0, lint 0, **241 test PASS**, dist `index.html` **240.82 KB** (~72.20 KB gz) vs baseline 263.18 KB / 80.26 KB gz → **−22.4 KB raw, −8.1 KB gz**; lucide eliminato dal bundle (0 KB).
