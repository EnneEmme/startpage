...
- **Custom JavaScript / bookmarklets** (`javascript:` URLs and `isScript`/`scriptContent` links) are a first-class feature. Execution policy:
  ...
  - Built-in default scripts (Unimib Orari/Esami) are first-party trusted bookmarklets with dynamic date and academic year calculation (`dynamicUrlRule` preserved for backward compatibility and fast fallback) and execute with implicit consent (`BUILTIN_SCRIPT_IDS`).
